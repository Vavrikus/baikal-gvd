...
int cascade_flux_val(int year = -1, int cluster = -1)
{
	...
	//path to data folder
	const char* env_p = "/home/vavrik/bajkal/recoCascades/v1.2";
	...
	for (int i = 0; i < reconstructedCascades.GetEntries(); ++i)
	{
		...
		{
			if(eventTime->GetSec() < 88918400) cout << "eventTime too low (" << *eventTime << ")\n";
			else flux_hist[hist_key]->Fill(*eventTime-788918400); //1970 unix to 1995 unix
		}
